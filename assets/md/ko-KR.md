# Simple Easy Social Login (SESLP) — 소셜 로그인 가이드 (한국어)

> 이 문서는 **Simple Easy Social Login (SESLP)** 플러그인에서  
> 각 소셜 로그인 공급자(Google, Facebook, LinkedIn, Naver, Kakao, Line)를 설정하는 방법을 안내합니다.  
...
- **HTTPS 필수** (HTTP 환경에서는 대부분 오류 발생)
- **Client ID / Secret** 은 각 공급자 콘솔에서 발급받아 SESLP 설정 페이지에 입력
- **정확한 일치**: Redirect URI는 콘솔에 등록된 값과 **100% 일치**해야 함  
  (프로토콜, 서브도메인, 쿼리 포함)
- **이메일 비공개 사용자 처리**: 일부 공급자는 사용자가 이메일 제공을 거부할 수 있음 →  
  플러그인은 내부적으로 ID 기반 사용자 연결 지원
- **로그 확인 경로**
...
- Redirect URI는 콘솔에 등록한 값과 **100% 일치**해야 합니다.  
  예) `https://example.com/?social_login=google`
- 테스트 모드에서는 **테스트 사용자**만 로그인 가능(최대 100명).
- 앱 홈페이지/개인정보처리방침/약관 URL을 사용하는 경우, **앱 도메인(Authorized domains)** 등록 및 **소유권 검증** 필요할 수 있음.
...
6. **앱 도메인(App domain)** 섹션
   ...
   - _필요 시_ Search Console을 통한 **도메인 소유권 검증** 진행.
7. **범위(Scopes)** 설정
...
<details>
<summary><strong>Facebook (메타)</strong></summary>
...
- **App Domains:** `example.com` (앱 정책/약관/홈페이지 URL의 도메인)
- **Privacy Policy URL:** 공개 접근 가능한 정책 페이지
- **Terms of Service URL:** 공개 접근 가능한 약관 페이지
- **User Data Deletion:** 지침 URL 또는 삭제 엔드포인트 제공
- **Category / App Icon:** 적절히 설정 후 **Save**

### 3) 스코프(권한) 및 App Review

- 일반 로그인에 필요한 기본 권한은 **`public_profile`**, 선택적 이메일은 **`email`**
- 대부분 **`email`은 검수 없이 사용 가능**하나 지역/계정별 예외가 있을 수 있음
- 페이지/광고 등 **고급 권한**은 **App Review** 및 **Business Verification** 필요
...
4. 여러 환경 사용 시 각각 등록
   - 로컬: `https://localhost:3000/?social_login=linkedin`
   - 스테이징: `https://staging.example.com/?social_login=linkedin`
   - 운영: `https://example.com/?social_login=linkedin`
5. **Save** 클릭
...
3. **Redirect URI**는 등록한 값과 **100% 일치** 확인
...
- [ ] 네이버 개발자센터 앱 등록 완료
- [ ] **Callback URL** 정확히 등록 (`https://{도메인}/?social_login=naver`)
- [ ] **HTTPS** 환경 사용
- [ ] 서브도메인별 별도 등록 (필요 시)
- [ ] Client ID / Secret → SESLP 입력 및 저장
- [ ] 이메일 동의/거부 시 동작 테스트
...
3. **Redirect URI 등록** 섹션
   ...
   - **저장** 클릭
4. **도메인 일치 확인**:  
   → Redirect URI의 도메인 부분이 **플랫폼 등록 도메인과 100% 일치**해야 함
...
3. **사이트 도메인** 입력: `https://{도메인}`
   - 예: `https://example.com`
4. **저장**
   > **Redirect URI 도메인과 정확히 일치**해야 인증 성공
...
- **invalid_client**  
  → Channel Secret 오타 또는 **Published 안 됨**
- **email NULL**  
...
| 플랜 | 공급자       | 필수 스코프 / 권장 스코프                            | Redirect URI 예시                         | 비고                      |
| ---- | ------------ | ---------------------------------------------------- | ----------------------------------------- | ------------------------- |
| 무료 | **Google**   | `openid email profile`                               | `https://{도메인}/?social_login=google`   | 외부 앱 동의 화면 필요    |
| 무료 | **Facebook** | `public_profile`, `email`                            | `https://{도메인}/?social_login=facebook` | `openid` 미사용           |
| 무료 | **LinkedIn** | `openid profile email`                               | `https://{도메인}/?social_login=linkedin` | OIDC 완전 전환            |
| 유료 | **Naver**    | `email`, `name`                                      | `https://{도메인}/?social_login=naver`    | “네아로” API 사용         |
| 유료 | **Kakao**    | `profile_nickname`, `profile_image`, `account_email` | `https://{도메인}/?social_login=kakao`    | Client Secret 활성화 필요 |
| 유료 | **Line**     | `openid profile email`                               | `https://{도메인}/?social_login=line`     | Publish 상태 필수         |
