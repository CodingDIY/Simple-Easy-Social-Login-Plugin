# Simple Easy Social Login (SESLP) — Social Login Guide (English)

> This document explains how to configure each social login provider  
> (Google, Facebook, LinkedIn, Naver, Kakao, LINE) in the **Simple Easy Social Login (SESLP)** plugin.  
...
- **HTTPS recommended/essential** (Use trusted development certificates for local environments).
- Redirect URI must **exactly match 100%** the value registered in the console.  
  Example) `https://example.com/?social_login=google`
- In test mode, only **test users** can log in (up to 100 users).
- When using app homepage/privacy policy/terms URLs, **app domains (Authorized domains)** registration and **ownership verification** may be required.
...
5. Enter **App Information**
   - App name, user support email, (optional) logo.
6. **App domain** section
   ...
   - _If needed_, perform **domain ownership verification** via Search Console.
7. Configure **Scopes**
...
- **App Domains:** `example.com` (the domain of the app’s policy/terms/homepage URL)
- **Privacy Policy URL:** Publicly accessible policy page
- **Terms of Service URL:** Publicly accessible terms page
- **User Data Deletion:** Provide a guideline URL or a data deletion endpoint
- **Category / App Icon:** Set appropriately, then **Save**

### 3) Scopes (Permissions) and App Review

- The basic permissions required for standard login are **`public_profile`**; the optional email is **`email`**
- In most cases, **`email` can be used without review**, but there may be exceptions depending on region/account
- **Advanced permissions** such as for pages/ads require **App Review** and **Business Verification**
...
- **Can't Load URL / redirect_uri error**  
...
> ※ LinkedIn is **phasing out** legacy scopes (`r_liteprofile`, `r_emailaddress`). New apps **must use OIDC standard scopes**.
...
> **Security:** Never expose Client Secret  
...
> **Note:** SESLP fully supports **OIDC flow**.  
...
> **Note:** SESLP fully supports **Naver Login (네아로)**.  
...
- **redirect_uri_mismatch** → 100% match required
- **invalid_client** → Secret not activated or typo
- **email empty** → User declined or unverified
- **Domain mismatch** → Platform vs Redirect URI
- **HTTP forbidden** → **HTTPS only**
...
- **redirect_uri_mismatch** → Even slight difference causes error → **100% match**
- **invalid_client** → Secret typo or **not Published**
- **email NULL** → **Email permission not approved** or user declined
- **HTTP not allowed** → **HTTPS required** (localhost HTTPS OK)
- **Development mode limit** → Only test accounts can log in
...
> **Note:** SESLP fully supports **LINE Login v2.1 + OpenID Connect**.  
...
| Plan | Provider     | Required / Recommended Scopes                        | Redirect URI Example                      | Notes                   |
| ---- | ------------ | ---------------------------------------------------- | ----------------------------------------- | ----------------------- |
| Free | **Google**   | `openid email profile`                               | `https://{domain}/?social_login=google`   | External consent screen |
| Free | **Facebook** | `public_profile`, `email`                            | `https://{domain}/?social_login=facebook` | `openid` not used       |
| Free | **LinkedIn** | `openid profile email`                               | `https://{domain}/?social_login=linkedin` | Full OIDC migration     |
| Paid | **Naver**    | `email`, `name`                                      | `https://{domain}/?social_login=naver`    | Uses “Naver Login” API  |
| Paid | **Kakao**    | `profile_nickname`, `profile_image`, `account_email` | `https://{domain}/?social_login=kakao`    | Client Secret required  |
| Paid | **LINE**     | `openid profile email`                               | `https://{domain}/?social_login=line`     | Must be Published       |
