...
- 새로운 사용자 계정은 소셜 제공자의 이메일 주소와 일치하는 기존 WordPress 사용자가 없을 때**만** 생성됩니다.
- 사용자 생성은 다음을 완전히 존중합니다:
  - WordPress 핵심 설정: **설정 → 일반 → 누구나 사용자 등록 가능**
  - 플러그인 설정: **자동 사용자 생성** (관리자 설정에서 비활성화 가능)
- 로그인 성공 후, 플러그인은 공식 WordPress `wp_login` 액션을 실행합니다. 이는 로그인 시도 모니터링, 2FA 적용, 인증 이벤트 로깅 등을 수행하는 보안 플러그인과의 완전한 호환성을 보장합니다.
- 자동 사용자 생성을 끄면, 동일한 이메일로 이미 계정이 있는 사용자만 소셜 제공자를 통해 로그인할 수 있습니다.
